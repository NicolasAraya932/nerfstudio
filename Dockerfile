...
RUN python3.10 -m pip install --no-cache-dir --upgrade pip setuptools==69.5.1 wheel && \
    python3.10 -m pip install --no-cache-dir torch==2.1.2+cu118 torchvision==0.16.2+cu118 'numpy<2.0.0' --extra-index-url https://download.pytorch.org/whl/cu118 && \
    ...
    python3.10 -m pip install --no-cache-dir pycolmap==0.6.1 pyceres==2.1 omegaconf==2.3.0
